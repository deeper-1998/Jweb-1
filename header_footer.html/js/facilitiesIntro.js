...
function mediaMenuLeave() {   //메뉴박스 가리기
    ...
    if (mediaMenu[0].style.display == "none") {
        console.log("hg");
        body.style.background = "linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url(images/mainImg.jpg)"
    }
}
//반응형 메뉴박스
...
$(function() {
    $(window).on("scroll", function() {
        if($(this).scrollTop() > 300) {
            $(".quickBtn").fadeIn("fast");
        } else {
            $(".quickBtn").fadeOut("fast");
        }
    });
...
$(function() {
    ...
    $(window).on('scroll', function() {
      var val = $(document).height() - $(window).height() - footHeight;
  
      if ($(window).scrollTop() >= val) {
          $sideBtn.addClass(".sideBtnStop")
      } else {
        $sideBtn.removeClass(".sideBtnStop")
      }
    });
  });
